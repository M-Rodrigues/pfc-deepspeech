...
def rnn_impl_grublockcell(x, seq_length, previous_state, reuse, cell_name=None):
    if cell_name is None:
        cell_name = 'cudnn_lstm/rnn/multi_rnn_cell/cell_0'

    with tfv1.variable_scope(cell_name):
        fw_cell = tf.contrib.rnn.GRUBlockCellV2(Config.n_cell_dim,
                                                reuse=reuse,
                                                name='cudnn_compatible_gru_cell')
...
def create_model(batch_x, seq_length, dropout, reuse=False, batch_size=None, previous_state=None, overlap=True, rnn_impl=rnn_impl_lstmblockfusedcell):
    ...
    layer_lstm_3, layer_lstm_state_3 = rnn_impl(layer_lstm_2, seq_length, previous_state[2], reuse, 'cudnn_lstm/rnn/multi_rnn_cell/cell_2')
    layer_lstm_4, layer_lstm_state_4 = rnn_impl(layer_lstm_3, seq_length, previous_state[3], reuse, 'cudnn_lstm/rnn/multi_rnn_cell/cell_3')
    output, output_state = rnn_impl(layer_lstm_4, seq_length, previous_state[4], reuse, 'cudnn_lstm/rnn/multi_rnn_cell/cell_4')
    ...
    layers['rnn_output_3'] = layer_lstm_3
    layers['rnn_output_state_3'] = layer_lstm_state_3
    layers['rnn_output_4'] = layer_lstm_4
    layers['rnn_output_state_4'] = layer_lstm_state_4
    ...
    with tfv1.Session(config=Config.session_config) as session:
        ...
        try:
            for epoch in range(FLAGS.epochs):
                ...
                log_entries.append(['Training', epoch, train_loss])
                ...
                if FLAGS.dev_files:
                    ...
                    log_entries.append(['Validation', epoch, dev_loss])                    
        ...
        with open('CVData/LOG_{}.json'.format(Config.n_hidden_1), 'w', encoding='utf-8') as json_file:
...
def create_inference_graph(batch_size=1, n_steps=16, tflite=False):
    ...
    if batch_size <= 0:
        ...
        previous_state_c_4 = tfv1.placeholder(tf.float32, [batch_size, Config.n_cell_dim], name='previous_state_c_4')
        previous_state_h_4 = tfv1.placeholder(tf.float32, [batch_size, Config.n_cell_dim], name='previous_state_h_4')
        previous_state_4 = tf.nn.rnn_cell.LSTMStateTuple(previous_state_c_4, previous_state_h_4)

        previous_state_c_5 = tfv1.placeholder(tf.float32, [batch_size, Config.n_cell_dim], name='previous_state_c_5')
        previous_state_h_5 = tfv1.placeholder(tf.float32, [batch_size, Config.n_cell_dim], name='previous_state_h_5')
        previous_state_5 = tf.nn.rnn_cell.LSTMStateTuple(previous_state_c_5, previous_state_h_5)

        previous_state = [previous_state, previous_state_2, previous_state_3, previous_state_4, previous_state_5]
    ...
    inputs = {
        ...
        'previous_state_c_4': previous_state_c_4,
        'previous_state_h_4': previous_state_h_4,
        'previous_state_c_5': previous_state_c_5,
        'previous_state_h_5': previous_state_h_5,
...
def do_single_file_inference(input_file_path):
    with tfv1.Session(config=Config.session_config) as session:
        ...
        previous_state_c_4 = np.zeros([1, Config.n_cell_dim])
        previous_state_h_4 = np.zeros([1, Config.n_cell_dim])
        previous_state_c_5 = np.zeros([1, Config.n_cell_dim])
        previous_state_h_5 = np.zeros([1, Config.n_cell_dim])
        ...
        logits = outputs['outputs'].eval(feed_dict={
            ...
            inputs['previous_state_c_4']: previous_state_c_4,
            inputs['previous_state_h_4']: previous_state_h_4,
            inputs['previous_state_c_5']: previous_state_c_5,
            inputs['previous_state_h_5']: previous_state_h_5,
        }, session=session)
